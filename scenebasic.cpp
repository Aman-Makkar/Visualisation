...
void SceneBasic::CreateVBO()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, 108 * sizeof(float) + sizeof(linearr), positionData, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, 108 * sizeof(float), positionData);
    glBufferSubData(GL_ARRAY_BUFFER, 108 * sizeof(float), sizeof(linearr), linearr);

     // bind colorBufferHandle to GL_ARRAY_BUFFER target
    glBindBuffer(GL_ARRAY_BUFFER, colorBufferHandle);
    glBufferData(GL_ARRAY_BUFFER, 108 * sizeof(float) + sizeof(linearr), colorData, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, 108 * sizeof(float), colorData);
    glBufferSubData(GL_ARRAY_BUFFER, 108 * sizeof(float), sizeof(linearr), linecolor);
...
void SceneBasic::cubeRotation(float vecbX, float vecbY, float vecbZ, float vecdX, float vecdY, float vecdZ, float alpha) {

    linearr[0] = vecbX-vecdX;
    linearr[1] = vecbY-vecdY;
    linearr[2] = vecbZ-vecdZ;
    linearr[3] = vecbX+vecdX;
    linearr[4] = vecbY+vecdY;
    linearr[5] = vecbZ+vecdZ;

    vecB = vec3(vecbX, vecbY, vecbZ);

    model = glm::translate(model, -this->vecB);

    vecD = glm::normalize(vec3(vecdX, vecdY, vecdZ));

    model = glm::rotate(model, alpha, this->vecD);
...
void SceneBasic::updateView(float eyeX, float eyeY, float eyeZ, float directX, float directY, float directZ) {
    view = glm::lookAt(vec3(eyeX,eyeY,eyeZ), vec3(directX,directY,directZ), vec3(0.0f,1.0f,0.0f));
}
